...
export default function MovieDetailsPage() {
  ...
  const fetchRelatedMovies = useCallback(async (currentMovie) => {
    ...
    try {
      // Backend ranks by shared-genre overlap first, then rating, and
      // backfills with top-rated movies if there aren't enough genre
      // matches — so this always returns a full row when the catalog
      // has enough movies, not just the 1-2 that share an exact genre.
      const res = await getRelatedMovies(currentMovie.id, { limit: 12 });
      const list = Array.isArray(res.data) ? res.data : (res.data?.movies ?? res.data?.data ?? []);
...
const mine = list.find(
  ...
  const handleShuffleRelated = useCallback(async () => {
    if (movie?.id) {
        ...
        <div className="md-nav-right">
          ...
          <div style={{ position: "relative" }} ref={avatarRef}>
            ...
            >
              ...
              <span className="md-nav-username" style={{ cursor: "pointer" }}>{displayName}</span>
            </button>
            {avatarOpen && (
              <div
                style={{
      ...
      >
        ...
        {!isLoading && status === "success" && movie && (
          <MovieContent
            ...
            onShuffleRelated={handleShuffleRelated}
          />
        )}
      </main>
...
function MovieContent({
  ...
  relatedMovies, relatedStatus, onShuffleRelated,
}) {
        ...
        {(relatedStatus === "loading" || relatedMovies.length > 0) && (
          <section className="md-section md-related-section">
            <div className="md-section-header">
              ...
              <button
                type="button"
                onClick={onShuffleRelated}
                disabled={relatedStatus === "loading"}
                className="md-shuffle-btn"
                title="Shuffle recommendations"
              >
                <RefreshCw size={14} strokeWidth={2} className={relatedStatus === "loading" ? "md-spin" : ""} />
                Shuffle
              </button>
            </div>
